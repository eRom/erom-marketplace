...
| `erom-dev-plugin` | [eRom/erom-agence-dev-plugin](https://github.com/eRom/erom-agence-dev-plugin) | `scaffold`, `illustrate`, `release` | Le cycle de vie d'un plugin Claude Code eRom : `scaffold` monte la structure du dépôt sans jamais écraser un fichier existant, `illustrate` dessine sa carte de présentation au fusain depuis l'inventaire réel lu par `claude plugin details` dans une composition dérivée de ce que le plugin résout, `release` publie une version en tenant l'ordre plugin puis marketplace et vérifie la CI |
| `erom-seo` | [eRom/erom-agence-seo](https://github.com/eRom/erom-agence-seo) | `audit`, `strategy`, `build`, `checklist`, `console`, `rapport` | Le cycle SEO/GEO d'un site sans abonnement tiers : `strategy` produit le contrat du site par interview, `audit` en fait le relevé daté de la seule URL jusqu'au code lancé en local, `build` corrige le code une trouvaille par commit, `checklist` tient les quinze cases avant et après la mise en ligne et les jalons J+1 à J+90, `console` lit Search Console et Bing et soumet le sitemap aux deux moteurs avec le ping IndexNow, `rapport` sort le livrable client en HTML autonome et imprimable. Chaque vérification cite mot pour mot la documentation officielle du moteur concerné |
| `erom-vision` | [eRom/erom-agence-vision](https://github.com/eRom/erom-agence-vision) | `gate-vision` | Le gate design avant l'œil de Romain : un juge vision externe évalue les captures d'un rendu UI, une par état d'interaction, contre la grille eRom du profil DS actif (perso dark-first ou institut papier/encre) et rend PASS/FAIL sur 10 critères binaires, chaque échec cité avec la preuve vue dans l'image et une suggestion. C1, C2 et C9 sont éliminatoires. Sans le juge, le gate affiche un bloc d'échec explicite plutôt qu'un PASS silencieux |
| `erom-dev-ios-apps` | [eRom/erom-agence-dev-ios-apps](https://github.com/eRom/erom-agence-dev-ios-apps) | `ios-app-intents`, `ios-debugger-agent`, `ios-simulator-browser`, `ios-ettrace-performance`, `ios-memgraph-leaks`, `swiftui-liquid-glass`, `swiftui-performance-audit`, `swiftui-ui-patterns`, `swiftui-view-refactor` | Construire, déboguer et mesurer une app iOS native en SwiftUI, le simulateur sous les yeux : App Intents et App Shortcuts, patterns et refactor de views, Liquid Glass, build et debug sur Simulator via le serveur MCP XcodeBuildMCP embarqué, miroir du Simulator dans le Browser pane de Desktop ou dans Chrome, audit de performance, profils ETTrace et memgraphs de fuites. Skills traduites et adaptées de build-ios-apps (OpenAI, MIT) |
| `erom-dev-macos-apps` | [eRom/erom-agence-dev-macos-apps](https://github.com/eRom/erom-agence-dev-macos-apps) | `swiftui-patterns`, `window-management`, `liquid-glass`, `view-refactor`, `appkit-interop`, `build-run-debug`, `swiftpm-macos`, `test-triage`, `telemetry`, `signing-entitlements`, `packaging-notarization` (+ au slash : `build-and-run-macos-app`, `test-macos-app`, `fix-codesign-error`) | Construire, lancer, tester et livrer une app macOS native en SwiftUI : scenes, windows, Liquid Glass et pont AppKit, build et run via un script `build_and_run.sh` unique, tri des tests et télémétrie `Logger`, signature, entitlements et notarization. Skills traduites et adaptées de build-macos-apps (OpenAI, MIT) |
